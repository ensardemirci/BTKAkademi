import sys
from PyQt5 import QtWidgets
from PyQt5.QtWidgets import QApplication,QMainWindow, QToolTip
from PyQt5.QtGui import QIcon
class MyWindow(QMainWindow):
    def __init__(self):
        super(MyWindow, self).__init__()

        self.setWindowTitle('Calculator')
        self.setGeometry(200,200,700,700)
        self.setWindowIcon(QIcon('../icon.png'))
        self.setToolTip('Prototip Calculator')
        self.initUI()

    def initUI(self):

        self.lbl_name = QtWidgets.QLabel(self)
        self.lbl_name.setText('Sayı1: ')
        self.lbl_name.move(50, 30)

        self.lbl_surname = QtWidgets.QLabel(self)
        self.lbl_surname.setText('Sayı2: ')
        self.lbl_surname.move(50, 70)

        self.lbl_result = QtWidgets.QLabel(self)
        self.lbl_result.resize(300,50)
        self.lbl_result.move(150,130)

        self.txt_name = QtWidgets.QLineEdit(self)
        self.txt_name.move(150, 30)
        self.txt_name.resize(200,32)

        self.txt_surname = QtWidgets.QLineEdit(self)
        self.txt_surname.move(150, 70)
        self.txt_surname.resize(200,32)


        self.btn_save = QtWidgets.QPushButton(self)
        self.btn_save.setText('Topla')
        self.btn_save.move(150, 110)
        self.btn_save.clicked.connect(self.clicked)

        self.btn_save = QtWidgets.QPushButton(self)
        self.btn_save.setText('Çıkar')
        self.btn_save.move(250, 110)
        self.btn_save.clicked.connect(self.clicked)

        self.btn_save = QtWidgets.QPushButton(self)
        self.btn_save.setText('Çarp')
        self.btn_save.move(350, 110)
        self.btn_save.clicked.connect(self.clicked)

        self.btn_save = QtWidgets.QPushButton(self)
        self.btn_save.setText('Böl')
        self.btn_save.move(450, 110)
        self.btn_save.clicked.connect(self.clicked)

    def clicked(self):
        sender = self.sender()
        print(sender.text())
        result = 0
        if sender.text() == 'Topla':
            result = float(self.txt_name.text()) + float(self.txt_surname.text())
            print(result)
            self.lbl_result.setText(f'Sonuç: {str(result)}')
        elif sender.text() == 'Çıkar':
            result = float(self.txt_name.text()) - float(self.txt_surname.text())
            print(result)
            self.lbl_result.setText(f'Sonuç: {str(result)}')

        elif sender.text() == 'Çarp':
            result = float(self.txt_name.text()) * float(self.txt_surname.text())
            print(result)
            self.lbl_result.setText(f'Sonuç: {str(result)}')

        elif sender.text() == 'Böl':
            result = float(self.txt_name.text()) / float(self.txt_surname.text())
            print(result)
            self.lbl_result.setText(f'Sonuç: {str(result)}')
        else:
            pass


def window():
    app = QApplication(sys.argv)
    win = MyWindow()
    win.show()
    sys.exit(app.exec_())
window()