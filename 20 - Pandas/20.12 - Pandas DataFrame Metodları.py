...
result = df
result = df["Column2"].unique() # farklı olan veriler
result = df["Column2"].nunique() # farklı olan verilerden oluşan yeni lsitenin sayısı
result = df["Column2"].value_counts() # hangi veriden kaç adet var
result = df["Column1"] * 2
result = df["Column1"].apply(kareal) # apply ile fonksiyon gönderebiliyoruz
